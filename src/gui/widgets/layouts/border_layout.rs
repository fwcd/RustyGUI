...
impl Layout for BorderLayout {
	fn arrange(&self, widgets: &mut Vec<LayoutedWidget>, parent_bounds: &WidgetBounds, graphics: &Graphics) {
		...
		let top_height = top.as_ref().map(|it| Self::height_of(it, max_height)).unwrap_or(0);
		let left_width = left.as_ref().map(|it| Self::width_of(it, max_width)).unwrap_or(0);
		let right_width = right.as_ref().map(|it| Self::width_of(it, max_width)).unwrap_or(0);
		let bottom_height = bottom.as_ref().map(|it| Self::height_of(it, max_height)).unwrap_or(0);
